  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!courseExist) {
        return next(
          new ErrorHandler(
            "You can't access this course without purchasing",
            404
          )
        );
      }

      const cousre = await CourseModel.findById(courseId);
      const content = cousre?.courseData;
      res.status(200).json({
        success: true,
        content,
      });
